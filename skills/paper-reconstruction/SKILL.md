...
8. 重建材料清单、事件/问卷日志、稳定 ID、数据字典、分析契约和验证测试；来源冲突必须进入验证计划。
...
10. 用户提出新想法时，区分原文支持、后续证据支持、用户推测与待验证部分；提炼可迁移参数并用当前证据校准。
...
- D 连接程序输出、事件日志、数据字典、分析契约和验证测试，跨文件名称通过语义审计。
- 正文、Supplement、OSF、数据或代码矛盾时保留版本差异，并把消解测试加入 D5。
- E 只沉淀从论文复现得到的参数与经文献校准的发展方向，不把用户想法冒充证据。
