...
| **C. 程序与现场协议** | 怎样运行与怎样分析？ | 原平台、目标平台原生组件、问卷逻辑、互动真实性、同步、降级、事件日志与**原文分析路线** |
| **D. 材料、数据与分析复现** | 怎样交接与验证？ | 来源/材料状态、最小数据字段、版本冲突、原文—输出差异与验证 |
| **E. 参数沉淀与研究发展** | 哪些结构可迁移？ | 参数卡、理论锚点、证据链缺口与有来源、有边界的新 idea |
